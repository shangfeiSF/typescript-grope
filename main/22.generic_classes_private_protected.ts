...
super_instance.not_achieve_private_super_method = function (x) {
    return this.private_super_prop.concat(x);
};
super_instance.not_achieve_protected_super_method = function (x, y) {
    if (y < x.length) {
        return x[y];
    }
...
console.log(super_instance.private_super_method(1));  // [1,2,3,4,5,1]
console.log(super_instance.protected_super_method([6, 7, 8, 9, 0], 2));  // 8